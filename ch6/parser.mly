...
exp :
  ...
  | FOR ID ASSIGN exp TO exp DO exp {A.ForExp {var=newSym $2; escape=ref false; lo=$4; hi=$6; body=$8; pos=left ()}}
...
tyfields :
    /* empty */ {[]}
  | ID COLON ID tyfields_ {{name=newSym $1; escape=ref false; typ=newSym $3; pos=leftPos 2} :: $4}
  ;

tyfields_ :
    /* empty */ {[]}
  | COMMA ID COLON ID tyfields_ {{name=newSym $2; escape=ref false; typ=newSym $4; pos=leftPos 3} :: $5}
  ;

vardec :
    VAR ID ASSIGN exp {A.VarDec {name=newSym $2; escape=ref false; typ=None; init=$4; pos=leftPos 2}}
  | VAR ID COLON ID ASSIGN exp {A.VarDec {name=newSym $2; escape=ref false; typ=Some (newSym $4, leftPos 4); init=$6; pos=leftPos 2}}
  ;
